...
SpoutControls::~SpoutControls()
{
	// MessageBoxA(NULL, "~SpoutControls OK", "Info", MB_OK);
	Cleanup();
}
...
bool SpoutControls::CreateControls(string mapname, vector<control> controls)
{
	...
	// printf("SpoutControls::CreateControls (%s)\n", mapname.c_str());
	...
	// printf("CreateControls map size = %d\n", m_dwSize);
	...
	// printf("CreateControls writing to map (%d)\n", controls.size());
...
bool SpoutControls::OpenControls(string mapname)
{
	...
	// printf("SpoutControls::OpenControls [%s]\n", m_sharedMemoryName.c_str());
	...
	// printf("WritePathToRegistry(%s) - Software\\Leading Edge\\Spout - ControlPath\n", Path);
	...
	// printf("WritePathToRegistry(%s) - Software\\Leading Edge\\Spout - ControlMap\n", Path);
	...
	if(filecontrols.size() > 0) {
		...
		// printf("OpenControls : filecontrols.size() = %d\n", filecontrols.size());
		...
		// printf("WritePathToRegistry(%s) - Software\\Leading Edge\\Spout - ControlFile\n", m_ControlFilePath);
		WritePathToRegistry(m_ControlFilePath, "Software\\Leading Edge\\Spout", "ControlFile");

		// Write the sender control file path to the registry
		// LJ DEBUG WritePathToRegistry(m_ControlFilePath, "Software\\Leading Edge\\Spout", m_sharedMemoryName.c_str());
...
bool SpoutControls::CloseControls()
{
	...
	// printf("SpoutControls::CloseControls()\n");
...
bool SpoutControls::OpenSpoutController(string CommandLine)
{
	...
	// printf("SpoutControls::OpenSpoutController()\n");

	// Has the right path been passed ?

	// MessageBoxA(NULL, Path, "Info 1", MB_OK);
	if(Path[0] != 0 && _access(Path, 0) != -1) {
		// MessageBoxA(NULL, "Path exists", "Info 2", MB_OK);
	}
	else {
		// MessageBoxA(NULL, "Path does not exist", "Info 2", MB_OK);
		// Is it an executable - if so is SpoutController.exe in the same folder
		if( GetModuleFileNameA(NULL, Path, MAX_PATH) ) {
			PathRemoveFileSpecA(Path);
			strcat_s(Path, MAX_PATH, "\\SpoutController.exe");
			// MessageBoxA(NULL, Path, "Info 3", MB_OK);
			// printf("%s\n", Path);
	...
	// MessageBoxA(NULL, Path, "Info 5", MB_OK);

	if(PathFileExistsA(Path) ) {
		// printf("%s\n", Path);
		// MessageBoxA(NULL, Path, "Info 6", MB_OK);
...
bool SpoutControls::CheckControls(vector<control> &controls)
{
	...
	if(CheckMail(m_sharedMemoryName, m_hSlot)) {
		// printf("SpoutControls::CheckControls - getting controls\n");
		// Get the new controls
		GetControls(controls);
		return true;
	}

	// printf("SpoutControls::CheckControls failed\n");
...
bool SpoutControls::CopyControlFile (string &filepath, string &destpath)
{
	...
	// printf("SpoutControls::CopyControlFile\nFrom : \n[%s]\nTo : \n[%s]\n", filepath.c_str(), destpath.c_str());
...
bool SpoutControls::CreateControlFile(const char *filepath)
{
	...
	// printf("SpoutControls::CreateControlFile\n(%s)\n", path);
...
bool SpoutControls::ReadControls(void *pBuffer, vector<control> &controls)
{
	...
	for(i = 0; i<nControls; i++) {
		...
		// if(control.type == 100) printf("%s : %s\n", control.name.c_str(), control.text);
...
{
	if(hSlot == NULL) {
		// printf("SpoutControls::CheckMail null mailslot\n");
		return false;
	}

	string slotmessage; // Not used but could return the message

	// printf("SpoutControls::CheckMail [%s] [%x]\n", SlotName, hSlot);
...
bool SpoutControls::ReadMail(string SlotName, HANDLE hSlot, string &SlotMessage)
{
    ...
    if( NULL == hEvent ) {
		// printf("SpoutControls::ReadMail CreateEvent failed\n");
		return false;
	}
	...
	// printf("SpoutControls::ReadMail returning OK\n");
...
bool SpoutControls::Cleanup()
{
	...
	// printf("SpoutControls::Cleanup()\n");
	// MessageBoxA(NULL, "SpoutControls Cleanup", "Info", MB_OK);
	...
	if(hwnd) {
		// printf("SpoutControls::Cleanup() - closing controller\n");
		...
		// MessageBoxA(NULL, "Closed SpoutController", "Info", MB_OK);
		// printf("SpoutControls::Cleanup() - closed controller OK\n");
	}
	else {
		// printf("SpoutControls::Cleanup() - controller not found\n");
		// MessageBoxA(NULL, "SpoutController not found", "Info", MB_OK);
	}
